...
class ChapterResource extends JsonResource
{
     ...
     public function toArray($request)
    {
        ...
        $chairperson = DB::table('volunteers')->join('users','volunteers.user_id','=','users.id')->join('positions','volunteers.position_id','=','positions.id')->where('volunteers.id' ,'=' ,$this->chairperson_id)->select('volunteers.id','users.firstName','users.lastName','positions.name')
        ->get();
        return [
            'id' => $this->id,
            'name' => $this->name,
